...
<div class="row">
    <div class="d-md-flex justify-content-md-end">
        <a class="btn btn-dark" href="{{route('movimientos-detalles.create')}}" role="button">Nuevo Registro</a>
    </div>
        <div class="card-body">
            ...
            <div class="table-responsive">
                ...
                <tbody>
                    @foreach ($datas as $data)
                        <tr>
                            <td>{{$data->movimientos_id}}</td>
                            <td>{{$data->productos_id}}</td>
                            <td>{{$data->nombre}}</td>
                            <td>{{$data->cantidad}}</td>
                            <td>{{$data->valor}}</td>
                            <td>
                                <div class="container text-center">
                                    ...
                                    <div class="col col-lg-2">
                                        <form action="{{ route('movimientos-detalles.destroy', $data->id) }}" method="post">
                                            @csrf
                                            @method('DELETE')
